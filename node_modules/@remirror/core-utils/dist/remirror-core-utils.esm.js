...
import { ErrorConstant, __INTERNAL_REMIRROR_IDENTIFIER_KEY__, LEAF_NODE_REPLACING_CHARACTER } from '@remirror/core-constants';
import { includes, isObject, isNumber, Cast, isString, findMatches, clamp, isArray, isNullOrUndefined, invariant, isEmptyObject, object, entries, isNonEmptyArray, assertGet, sort, uniqueBy, assert, keys, omit, unset, isFunction } from '@remirror/core-helpers';
import { Selection, EditorState, NodeSelection, Transaction, TextSelection, AllSelection } from '@remirror/pm/state';
import { AddMarkStep, ReplaceAroundStep, ReplaceStep, RemoveMarkStep, liftTarget, findWrapping } from '@remirror/pm/transform';
import _objectWithoutProperties from '@babel/runtime/helpers/esm/objectWithoutProperties';
import _slicedToArray from '@babel/runtime/helpers/esm/slicedToArray';
import { cssifyObject } from 'css-in-js-utils';
import { ResolvedPos, NodeType, MarkType, Node as Node$1, Fragment, Mark, Schema, DOMSerializer, DOMParser } from '@remirror/pm/model';
import parse from 'parenthesis';
import { CoreUtilsMessages } from '@remirror/messages';
...
function parseCSSFunction(cssFunc, unitConvertor) {
  ...
  var _iterator = _createForOfIteratorHelper(findMatches(firstToken, CSS_FUNCTION_REGEX)),
      _step;
        ...
        case 'clamp':
          {
            ...
            if (isNumber(min) && isNumber(_value) && isNumber(max)) {
              return clamp({
                min,
                max,
                value: _value
...
function isDomNode(domNode) {
  return typeof document !== 'undefined' && (isObject(Node) ? domNode instanceof Node : isObject(domNode) && isNumber(Cast(domNode).nodeType) && isString(Cast(domNode).nodeName));
}
/**
...
function findNodeAtSelection(selection) {
  ...
  !parentNode ? process.env.NODE_ENV !== "production" ? invariant(false, {
...
function getActiveNode(props) {
  ...
  var selection = state.selection,
      doc = state.doc;
  var nodeType = isString(type) ? doc.type.schema.nodes[type] : type;
...
function schemaToJSON(schema) {
  ...
  var _iterator2 = _createForOfIteratorHelper(entries(schema.nodes)),
      _step2;
  ...
  var _iterator3 = _createForOfIteratorHelper(entries(schema.marks)),
      _step3;
...
function mergeKeyBindingCreator(extensionKeymaps, mapper) {
  ...
  var mappedCommands = object(); // Outer loop iterates over each object keybinding.
  ...
  try {
    for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
      ...
      // `Cmd-Escape`.
      var _iterator5 = _createForOfIteratorHelper(entries(extensionKeymap)),
          _step5;
...
function isIdentifierOfType(value, type) {
  return isArray(type) ? includes(type, value[__INTERNAL_REMIRROR_IDENTIFIER_KEY__]) : type === value[__INTERNAL_REMIRROR_IDENTIFIER_KEY__];
}
/**
...
function isTransaction(value) {
  return isObject(value) && value instanceof Transaction;
}
/**
...
function isMarkActive(props) {
  ...
  var markType = isString(type) ? doc.type.schema.marks[type] : type;
...
function prosemirrorNodeEquals(node, other) {
  ...
  var identicalMarkup = node.type === other.type && Mark.sameSet(node.marks, other.marks);
...
function getMarkRange($pos, type, $end) {
  ...
  var typeName = isString(type) ? type : type.name; // Find the mark if it exists.
  ...
  var text = $pos.doc.textBetween(startPos, endPos, LEAF_NODE_REPLACING_CHARACTER, '\n\n');
  return {
    from: startPos,
    to: endPos,
    text,
...
function getChangedRanges(tr) {
  ...
  var sortedRanges = sort(rawRanges, (a, z) => a.from - z.from); // Merge sorted ranges into the new range.
...
function getSelectedGroup(state, exclude) {
  ...
  var getChar = (start, end) => getTextContentFromSlice(TextSelection.create(state.doc, start, end).content());
  ...
  var text = state.doc.textBetween(from, to, LEAF_NODE_REPLACING_CHARACTER, '\n\n');
  return {
    from,
    to,
    text
...
function getMatchString(match) {
  ...
  var value = isArray(match) ? match[index] : match; // Throw an error if value is not defined for the index.

  assert(isString(value), "No match string found for match ".concat(match));
...
function getTextSelection(selection, doc) {
  ...
  var clampToDocument = value => clamp({
    min,
    max,
    value
  });
  ...
  if (isAnchorHeadObject(pos)) {
    ...
    return TextSelection.create(doc, _anchor, _head);
...
function createDocumentNode(props) {
  ...
  !attemptsRemaining ? process.env.NODE_ENV !== "production" ? invariant(false, {
  ...
  if (isString(content)) {
    !stringHandler ? process.env.NODE_ENV !== "production" ? invariant(false, {
      code: ErrorConstant.INVALID_CONTENT,
  ...
  try {
    ...
    !transformedContent ? process.env.NODE_ENV !== "production" ? invariant(false, {
      code: ErrorConstant.INVALID_CONTENT,
...
function prosemirrorNodeToDom(node) {
  ...
  var fragment = isDocNode(node, node.type.schema) ? node.content : Fragment.from(node);
  return DOMSerializer.fromSchema(node.type.schema).serializeFragment(fragment, {
    document
  });
}
...
function areSchemasCompatible(schemaA, schemaB) {
  ...
  var marksA = keys(schemaA.marks);
  var marksB = keys(schemaB.marks);
  var nodesA = keys(schemaA.nodes);
  var nodesB = keys(schemaB.nodes);
...
function omitExtraAttributes(attrs, extra) {
  var extraAttributeNames = keys(extra.defaults());
  return omit(_objectSpread({}, attrs), extraAttributeNames);
}
/**
...
function joinStyles(styleObject, initialStyles) {
  ...
  var end = cssifyObject(styleObject);
...
function getDocRange(doc) {
  var _AllSelection = new AllSelection(doc),
...
function getInvalidContent(_ref4) {
  ...
  var validMarks = new Set(keys(schema.marks));
  var validNodes = new Set(keys(schema.nodes));
  ...
  if (marks) {
    ...
    try {
      for (_iterator8.s(); !(_step8 = _iterator8.n()).done;) {
        ...
        var name = isString(mark) ? mark : mark.type;
...
function updateMark(props) {
  return _ref => {
    ...
    var type = props.type,
        _props$attrs = props.attrs,
        attrs = _props$attrs === void 0 ? object() : _props$attrs,
        appendText = props.appendText,
        range = props.range;
    var selection = range ? TextSelection.create(tr.doc, range.from, range.to) : tr.selection;
    var $from = selection.$from,
        from = selection.from,
        to = selection.to;
...
function wrapIn(type) {
  ...
  return function (props) {
    var tr = props.tr,
        ...
        state = props.state;
    var nodeType = isString(type) ? assertGet(state.schema.nodes, type) : type;
    ...
    var wrapping = blockRange && findWrapping(blockRange, nodeType, attrs);
...
function toggleWrap(nodeType) {
  ...
  return props => {
    var tr = props.tr,
        state = props.state;
    var type = isString(nodeType) ? assertGet(state.schema.nodes, nodeType) : nodeType;
...
function replaceText(props) {
  ...
  return _ref3 => {
    ...
    var from = selection.from,
        to = selection.to,
        $from = selection.$from;
    var type = isString(props.type) ? (_schema$nodes$props$t = schema.nodes[props.type]) !== null && _schema$nodes$props$t !== void 0 ? _schema$nodes$props$t : schema.marks[props.type] : props.type;
    !(isString(props.type) ? type : true) ? process.env.NODE_ENV !== "production" ? invariant(false, {
      code: ErrorConstant.SCHEMA,
    ...
    if (isNodeType(type)) {
      ...
      !content ? process.env.NODE_ENV !== "production" ? invariant(false, {
...
function removeMark(props) {
  return _ref5 => {
    ...
    dispatch === null || dispatch === void 0 ? void 0 : dispatch(tr.removeMark(from, isNumber(to) ? to : from, isMarkType(markType) ? markType : undefined));
    return true;
  };
}
...
function findChildren(props) {
  ...
  !isProsemirrorNode(node) ? process.env.NODE_ENV !== "production" ? invariant(false, {
    code: ErrorConstant.INTERNAL,
    message: 'Invalid "node" parameter passed to "findChildren".'
  }) : invariant(false) : void 0; // Ensure that the predicate is a function.

  !isFunction(predicate) ? process.env.NODE_ENV !== "production" ? invariant(false, {
    code: ErrorConstant.INTERNAL,
    message: 'Invalid "predicate" parameter passed to "findChildren".'
  }) : invariant(false) : void 0; // This is used to keep track of all the node positions.
...
function findChildrenByAttribute(props) {
  ...
  function predicate(nodeWithPos) {
    var attributeKeys = new Set(keys(nodeWithPos.node.attrs));

    var _iterator = _createForOfIteratorHelper(entries(attrs)),
        _step;
...
function markInputRule(props) {
  ...
  var markType;
  var rule = new InputRule(regexp, (state, match, start, end) => {
    var tr = state.tr,
        schema = state.schema;
...
function nodeInputRule(props) {
  ...
  var rule = new InputRule(regexp, (state, match, start, end) => {
    ...
    var tr = state.tr,
        schema = state.schema;
    var nodeType = isString(type) ? schema.nodes[type] : type;
    var captureGroup = match[1];
    var fullMatch = match[0]; // These are the attributes which are added to the mark and they can be
    // obtained from the match if a function is provided.
...
function gatherDetails(_ref) {
  ...
  invalidMarks && markActiveInRange({
    $from,
    $to
  }, invalidMarks) || rule.invalidMarks && markActiveInRange({
    $from,
    $to
