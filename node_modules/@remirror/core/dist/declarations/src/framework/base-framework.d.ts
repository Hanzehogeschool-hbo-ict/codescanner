...
import type { EditorSchema, EditorState, EditorStateProps, EditorViewProps, PrimitiveSelection, RemirrorContentType, TextProps, Transaction, TransactionProps, TransactionTransformer } from '@remirror/core-types';
...
export declare type AddFrameworkHandler<Extension extends AnyExtension> = <Key extends keyof FrameworkEvents<Extension>>(event: Key, cb: FrameworkEvents<Extension>[Key]) => Unsubscribe;
/**
...
export declare type CreateStateFromContent<Extension extends AnyExtension> = (content: RemirrorContentType, selection?: PrimitiveSelection) => EditorState<GetSchema<Extension>>;
export interface RemirrorEventListenerProps<Extension extends AnyExtension> extends EditorStateProps<GetSchema<Extension>>, Remirror.ListenerProperties<Extension>, EditorViewProps<GetSchema<Extension>> {
    /**
...
export declare type RemirrorEventListener<Extension extends AnyExtension> = (params: RemirrorEventListenerProps<Extension>) => void;
export declare type AttributePropFunction<Extension extends AnyExtension> = (params: RemirrorEventListenerProps<Extension>) => Record<string, string>;
export interface PlaceholderConfig extends TextProps {
    className: string;
}
export interface UpdateStateProps<Schema extends EditorSchema = EditorSchema> extends Partial<TransactionProps<Schema>>, EditorStateProps<Schema>, TriggerChangeProps {
    /**
     * When the state updates are not controlled and it was a transaction that
     * caused the state to be updated this value captures all the transaction
     * updates caused by prosemirror plugins hook state methods like
...
export interface ListenerProps<Extension extends AnyExtension> extends Partial<EditorStateProps<GetSchema<Extension>>>, Partial<TransactionProps<GetSchema<Extension>>> {
    /**
     * When the state updates are not controlled and it was a transaction that
     * caused the state to be updated this value captures all the transaction
     * updates caused by prosemirror plugins hook state methods like
...
export declare type UpdatableViewPropsObject = {
